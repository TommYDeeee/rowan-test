...
use crate::{
    green::GreenTokenData, red, Direction, GreenNode, GreenNodeData, GreenToken, NodeOrToken,
...
impl<L: Language> SyntaxNode<L> {
    pub fn new_root(green: GreenNode) -> SyntaxNode<L> {
        SyntaxNode::from(red::SyntaxNode::new_root(green))
    }
...
impl<L: Language> From<red::SyntaxNode> for SyntaxNode<L> {
    fn from(raw: red::SyntaxNode) -> SyntaxNode<L> {
        SyntaxNode {
            raw,
            _p: PhantomData,
        }
...
impl<L: Language> From<SyntaxNode<L>> for red::SyntaxNode {
    fn from(node: SyntaxNode<L>) -> red::SyntaxNode {
        node.raw
    }
}
