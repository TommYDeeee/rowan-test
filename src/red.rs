//! Implementation of the cursors also called red trees
//! -- API for convenient access to syntax trees.
//!
...
impl NodeData {
    ...
    fn green(&self) -> GreenElementRef<'_> {
        match &self.green {
            Green::Node { ptr } => NodeOrToken::Node(unsafe { &*ptr.get().as_ptr() }),
            Green::Token { ptr } => NodeOrToken::Token(unsafe { &*ptr.as_ref() }),
        }
    }
