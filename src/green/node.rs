...
impl ToOwned for GreenNodeData {
    ...
    fn to_owned(&self) -> GreenNode {
        unsafe {
            let green = ManuallyDrop::new(GreenNode::from_raw(ptr::NonNull::from(self)));
...
impl GreenNode {
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        let children = children.into_iter().map(|el| {
            ...
            match el {
                NodeOrToken::Node(it) => GreenChild::Node {
                    rel_offset,
                    node: it,
                },
                NodeOrToken::Token(it) => GreenChild::Token {
                    rel_offset,
                    token: it,
                },
            }
        ...
        // Fix text_len after it is constructed because we cannot iterate children twice
        let data = {
...
impl GreenChild {
    ...
    fn rel_range(&self) -> TextRange {
        TextRange::at(self.rel_offset(), self.as_ref().text_len())
    }
}
...
impl<'a> DoubleEndedIterator for Children<'a> {
    ...
    {
        let mut accum = init;
        while let Some(child) = self.next_back() {
            accum = f(accum, child);
        }
        accum
    }
